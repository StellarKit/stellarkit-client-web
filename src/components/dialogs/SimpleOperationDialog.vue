  ...
  methods: {
    updateForOperation() {
      switch (this.operation) {
        case 'domain':
          ...
          this.header =
            'Adds a domain name to the source account. Leave blank to remove. Funding account is optional.'
          break
        case 'inflation':
          this.showSource = true
          this.showFunding = true
          ...
          this.header =
            'Adds the inflation destination to the source account. Funding account is optional.'
          this.subHeader =
            'ex: GCCD6AJOYZCUAQLX32ZJF2MKFFAUJ53PVCFQI3RHWKL3V47QYE2BNAUT'
    ...
    setInflationDestination() {
      ...
      if (sourceWallet && Helper.strOK(textValue)) {
        ...
        StellarUtils.setInflationDestination(
          sourceWallet,
          textValue,
          fundingWallet,
          additionalSigners
    ...
    federationLookup() {
      ...
      if (Helper.strOK(textValue)) {
        if (!Helper.isFederation(textValue)) {
          this.displayToast(
            'The address "' + textValue + '" appears to be invalid.',
            true
          )
        } else {
